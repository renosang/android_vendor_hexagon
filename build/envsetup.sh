function __print_hexagon_functions_help() {
cat <<EOF
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/HexagonRom-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop org.hexagon.device | grep -q "$HEXAGON_BUILD"); then
...
function hexagonremote()
{
    ...
    git remote add hexagon https://android.googlesource.com/$PFX$PROJECT
    echo "Remote 'hexagon' created"
}
...
function installboot()
{
    ...
    if (adb shell getprop org.hexagon.device | grep -q "$CUSTOM_BUILD");
    then
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "hexagongerrit" ]; then
...
function hexagonrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "hexagon Gerrit Rebase Usage: "
        echo "      hexagonrebase <path to project> <patch IDs on Gerrit>"
...
function dopush()
{
    ...
    if (adb shell getprop org.hexagon.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/hexagon/build/tools/repopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/hexagon/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
