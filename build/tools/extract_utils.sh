...
function oat2dex() {
    ...
    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$HEXAGON_ROOT"/vendor/hexagon/build/tools/"$HOST"/vdexExtractor
    fi
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$HEXAGON_ROOT"/vendor/hexagon/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
